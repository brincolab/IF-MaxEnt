    ...
    for size in [20, 40, 60, 80, 100, 120]:
        for r in [4, 5]:
            for T in [2, 6, 8, 9, 10, 11, 12, 15, 20, 30, 40]:
                ...
                H = np.loadtxt('./SHParams/150SWk4r.{}w.2sub{}H.txt'.format(r, size))
                J = np.loadtxt('./SHParams/150SWk4r.{}w.2sub{}J.txt'.format(r, size))
                E = ising_subproc(nb_steps, T*0.1, H, J)
                np.savetxt("Results/r{}T{}N{}E.txt".format(r,T,size),E) #Save the energy
    print('Done!')
